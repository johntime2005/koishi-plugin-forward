...
export function apply(ctx: Context, config: Config) {
	...
	function parseTarget(
		...
		const platforms = [...new Set(ctx.bots.map((b) => b.platform))].sort(
      ...
      return session.selfId
		...
		for (const bot of ctx.bots) {
			if (bot.platform === platform) {
				...
				if (
					bot.selfId === channelId ||
					(bot as any).config?.serverName === channelId
				) {
					return bot.selfId;
				}
				if (!fallbackBot) {
...
=======
	...
	ctx.on("internal/session", (session) => {
		...
		const name =
			...
			session.userId;
		let text: string;
>>>>>>> 21bea62 (fix: resolve incorrect bot id selection when adding rules for multiple minecraft servers)
...
>>>>>>> 21bea62 (fix: resolve incorrect bot id selection when adding rules for multiple minecraft servers)
	...
	if (config.mode === "database") {
		ctx.inject(["database"], (ctx) => {
			const cmd = ctx.command("forward", { authority: 3 }).alias("fwd");

			const register = (
				def: string,
				callback: (
					argv: { session: Session<never, "forward"> },
					...args: any[]
				) => Promise<any>,
			) =>
				cmd
					...
					.action((argv, ...args) => {
						...
						return callback(
							argv as { session: Session<never, "forward"> },
							...args,
						);
					});
				...
				if (
					targets.some(
						(t) =>
							...
							t.channelId === parsed.channelId,
					)
				) {
					return session.text(".unchanged", [
						formatTarget({ ...parsed, selfId }),
					]);
				}
...
=======
			...
			register(".remove <channel:channel>", async ({ session }, id) => {
				...
				const index = targets.findIndex(
					(t) =>
						t.platform === parsed.platform && t.channelId === parsed.channelId,
				);
				if (index >= 0) {
